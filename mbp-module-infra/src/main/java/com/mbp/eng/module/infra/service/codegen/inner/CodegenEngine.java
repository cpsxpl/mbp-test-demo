package com.mbp.eng.module.infra.service.codegen.inner;
...
import com.mbp.eng.framework.apilog.core.annotation.ApiAccessLog;
import com.mbp.eng.framework.apilog.core.enums.OperateTypeEnum;
...
import com.mbp.eng.framework.excel.core.convert.DictConvert;
import com.mbp.eng.framework.excel.core.util.ExcelUtils;
...
import com.mbp.eng.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.mbp.eng.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.mbp.eng.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.mbp.eng.module.infra.enums.codegen.CodegenSceneEnum;
import com.mbp.eng.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.mbp.eng.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.mbp.eng.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 mbp-cloud 项目,用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 mbp-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.mbp.eng.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/mbp-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/mbp-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "mbp-module-${table.moduleName}/" + // 顶级模块
                "mbp-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "mbp-module-${table.moduleName}/" + // 顶级模块
                "mbp-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "mbp-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "mbp-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "mbp-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "mbp-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
